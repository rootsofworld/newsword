...
pagelist.forEach(async page => {
    const postlist = JSON.parse(await readFile(inputDB+'/'+page,'utf8'));
    console.log(postlist.pagename)
    if(!pageMap.has(postlist.pagename)) console.error("Can't find " + postlist.pagename)
    const rule = pageMap.get(postlist.pagename).rules
    //console.log(rule)
    
    var LinkTypePosts = postlist.posts
    ...
    Promise.all(LinkTypePosts).then(data => {
        ...
        fs.writeFile(inputDB+'/'+page, JSON.stringify(result), 'utf8', function(err){
            if(err) throw err;
        })
        //console.log(Object.keys(result))
    })
  
 //console.log(LinkTypePosts[0])  
})*/
