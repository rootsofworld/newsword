...
//////////// 讀進所有postlist /////////////
let postlists = fs.readdirSync(`../data/${pack}/`).filter( filename => {
...
async function main(packSize){
    ...
    fs.writeFileSync(`../data/json/${since}-${until}/result-${since}-${until}-${pack}.json`, JSON.stringify({data:fanpageList}), 'utf8')
    ...
    let json2csvFormat = []
      ...
      fs.writeFile(`../data/csv/${since}-${until}/result-${since}-${until}-${pack}.csv`, csv, (err) => {
          if(err) throw err
...
function createFanpage(filename){
    return new Promise((resolve, reject) => {
        readFile(`../data/${pack}/` + filename,'utf8')
        .then(data => {
            data = JSON.parse(data)
            let tmp = new Fanpage(data.id, data.name, data.rules, data.posts)
            resolve(tmp)
        })
